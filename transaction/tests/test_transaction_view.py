...
class TransactionViewTest(TestCase):
    ...
    def test_list_view_2(self):
        TransactionInfo.objects.create(title=TransactionName.create_lesson_record)
        TransactionInfo.objects.create(title=TransactionName.create_lesson_record)
        view = TransactionViewSet.as_view({"get": ActionEnum.list.value})
        request = self.factory.get("/")
        force_authenticate(request, self.user)
    ...
    def test_list_view_3(self):
        ...
        TransactionInfo.objects.create(title=TransactionName.create_lesson_record, details=details)
        TransactionInfo.objects.create(title=TransactionName.create_lesson_record, details=details)
        view = TransactionViewSet.as_view({"get": ActionEnum.list.value})
        request = self.factory.get("/")
        force_authenticate(request, self.user)
    ...
    def test_create_view(self):
        ...
        request = self.factory.post("/", {"title": TransactionName.create_lesson_record})
        force_authenticate(request, self.user)
        response = view(request)
        self.assertEquals(response.status_code, HTTPStatus.CREATED)
        self.assertEquals(response.data["title"], TransactionName.create_lesson_record.value)
    ...
    def test_update_view(self):
        details = TransactionDetail(app_label=BookingConfig.name,
                                    model_name=BookingRecord.__name__,
                                    pk=1)
        info = TransactionInfo.objects.create(title=TransactionName.create_lesson_record, details=details)
        view = TransactionViewSet.as_view({"patch": ActionEnum.update.value})
        request = self.factory.patch("/", {"title": TransactionName.create_lesson_record, "details": details.to_dict()},
                                     format="json")
        force_authenticate(request, self.user)
    ...
    def test_delete_view(self):
        ...
        request = self.factory.delete("/", {"title": TransactionName.create_lesson_record, "details": details.to_dict()},
                                      format="json")
        force_authenticate(request, self.user)
        response = view(request, pk=info.pk)
